...
def load_yaml_config(config_file):
    ...
    try:
        ...
        if not isinstance(config, dict):
            logger.error(f"Invalid YAML structure in {config_file}: not a dictionary")
            return None
            
        if "spin_groups" not in config:
            logger.error(f"Missing spin_groups in {config_file}")
            return None
            
        if not isinstance(config["spin_groups"], dict):
            logger.error(f"Invalid spin_groups structure in {config_file}: not a dictionary")
            return None
...
def aggregate_versions(versions_dir):
    ...
    for yaml_file in sorted(yaml_files):
        ...
        if config is None:
            logger.warning(f"Skipping invalid file: {yaml_file}")
            continue
        
        for group_name, group_data in config["spin_groups"].items():
            if not aggregated_spins[group_name].get("content_id"):
                aggregated_spins[group_name]["content_id"] = group_data["content_id"]
